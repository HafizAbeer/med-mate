...
exports.signup = async (req, res) => {
    try {
        ...
        const { firstName, lastName, email, password, dateOfBirth, gender, phoneNumber, role: rawRole } = req.body;
        const role = ['patient', 'caretaker'].includes(rawRole) ? rawRole : 'patient';
...
exports.login = async (req, res) => {
    try {
        ...
        const populatedUser = await User.findById(user._id).populate('caretaker', 'name');
        ...
        res.status(200).json({
            ...
            user: {
                ...
                caretakerName: populatedUser.caretaker ? populatedUser.caretaker.name : null
            }
        });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
...
exports.forgotPassword = async (req, res) => {
    try {
        ...
        try {
            await sendEmail({
                email: user.email,
                subject: 'Med-Mate Password Reset',
